...
class ConvReluNorm(BaseModule):
    ...
    def forward(self, x, x_mask): #  x[B 192 L] x_mask[B 1 L]
        x_org = x
...
# 参照FastSpeech2 duration predictor设计， 
# The duration predictor consists of two
# convolutional layers followed by a projection layer
class DurationPredictor(BaseModule):
    # 192+64 if num_spk > 1， 256，3， 0.1
...
# 参照Transformer Encoder设计, 六层， Multi-Head Attention, Add&Norm, FFN, Add&Norm
# x --> 1(x)-->x+1(x)--2(x+1(x))--x+1(x) + 2(x+1(x))
class Encoder(BaseModule):
    # 192+64 if num_spk > 1,  768, 2, 6, 
    # 3, 0.1, 4
    ...
    def forward(self, x, x_mask): # x[B 192 L] x_mask[B 1 L]
        attn_mask = x_mask.unsqueeze(2) * x_mask.unsqueeze(-1) # [B, 1, L, L]
        for i in range(self.n_layers):
            x = x * x_mask
...
# The encoder is composed of a pre-net, 6 Transformer blocks 
# with multi-head self-attention, and the final linear projection layer
class TextEncoder(BaseModule):
    # n_vocab=149 , n_feats=80, n_enc_channels=192, filter_channels=768, 
    # filter_channels_dp=256, n_heads=2, n_enc_layers=6, enc_kernel=3, enc_dropout=0.1
    # window_size=4
    def __init__(self, n_vocab, n_feats, n_channels, filter_channels,
                 filter_channels_dp, n_heads, n_layers, kernel_size, 
                 p_dropout, window_size=None, spk_emb_dim=64, n_spks=1, use_finer=False):
        super(TextEncoder, self).__init__()
        self.n_vocab = n_vocab
        self.n_feats = n_feats
        self.n_channels = n_channels            #  192
        ...
        # 192+64 if num_spk > 1,  768, 2, 6, 
        # 3, 0.1, 4
        self.encoder = Encoder(n_channels + 0, filter_channels, n_heads, n_layers, 
                               kernel_size, p_dropout, window_size=window_size)
        
        # 192+64 if num_spk > 1， 80， 1, 将文本编码特征变换为80维，文本与mel进行MAS
        self.proj_m = torch.nn.Conv1d(n_channels + 0 , n_feats, 1)
        # 192+64 if num_spk > 1， 256，3， 0.1
        self.proj_w = DurationPredictor(n_channels + (spk_emb_dim), filter_channels_dp, 
                                        kernel_size, p_dropout)

    def forward(self, x, x_lengths, spk=None, emo_finer=None):
        x = self.emb(x) * math.sqrt(self.n_channels) # B, L, n_channels=192
        x = torch.transpose(x, 1, -1)                # B, n_channels=192, L
        x_mask = torch.unsqueeze(sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)

        x = self.prenet(x, x_mask) # return x[B 192 L]
        # if self.n_spks > 1 and spk is not None:
        #     x = torch.cat([x, spk.unsqueeze(-1).repeat(1, 1, x.shape[-1])], dim=1)

        # if self.use_finer:
        # print(f'emo_finer: {emo_finer.shape}')
        x = x + emo_finer

        x = self.encoder(x, x_mask) # return x[B 192 L]
        mu = self.proj_m(x) * x_mask # [B, 80, L] * [B, 1, L] --> [B, 80, L]

        x_dp = torch.detach(x)
        spk = spk.unsqueeze(-1).repeat(1, 1, x_dp.shape[-1])
        # pdb.set_trace()
        # print(x_dp.shape)
        # print(spk.shape)
        # print(emo_finer.shape)

        x_dp = torch.cat([x, spk], dim=1)
        # print(x_dp.shape)
        logw = self.proj_w(x_dp, x_mask) # return logw[B, 1, L]

        return mu, logw, x_mask
    
    def get_ling_infos(self,x, x_lengths):
    # def forward(self, x, x_lengths, spk=None, emo_finer=None):
        x = self.emb(x) * math.sqrt(self.n_channels) # B, L, n_channels=192
        x = torch.transpose(x, 1, -1)                # B, n_channels=192, L
        x_mask = torch.unsqueeze(sequence_mask(x_lengths, x.size(2)), 1).to(x.dtype)
